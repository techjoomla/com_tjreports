				...
				<tr>
					<?php
							foreach ($colToshow as $eachColumn)
							{
								...
								$sortCol = $mainframe->getUserState( "com_tjreports.attemptreport_table_sortCol", 'id' );
								$sortOrder = $mainframe->getUserState( "com_tjreports.attemptreport_table_sortOrder", 'asc' );
								...
								if ($sortCol == $eachColumn && !empty($sortCol) && !empty($sortOrder) )
								{
									...
									?>
									<th class="th_<?php echo $eachColumn; ?> left <?php if(!empty($class)){ echo $class; } ?>">
										<?php $calHeading = strtoupper($eachColumn); ?>
										<?php $calHeading = 'PLG_TJREPORTS_ATTEMPTREPORT_' . $calHeading; ?>

										<a href="#" onclick="sortColumns(this); return false;" class="hasTooltip" title="<?php echo JText::_('PLG_TJREPORTS_ATTEMPTREPORT_SORT'); ?>" data-value="<?php echo $eachColumn; ?>"><?php echo JText::_($calHeading); ?><?php echo $icon; ?></a>
										...
										if (in_array($eachColumn,$showSearchToCol))
										{
											if ($eachColumn == 'name')
												{
													if (isset($this->lessonFilter))
													{
														echo JHtml::_('select.genericlist', $this->lessonFilter, "search-filter-". $eachColumn , 'class="filter-input input-medium" size="1" onchange="getFilterdata();" name="filter_lessonfilter"', "value", "text", isset($filters[$eachColumn]) ? $filters[$eachColumn] : '');
													}
												}
